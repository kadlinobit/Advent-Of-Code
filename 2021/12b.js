...
function findPath(path, nextNode) {
    // #1 exclude the possibility of visiting start node again
    if(path.length > 0 && nextNode === 'start') {
        return
    }
    // #2 Next node is end => path is finished, push it to the finalPaths array
    if(nextNode === 'end') {
        path.push(nextNode)
        finalPaths.push(path)
        return
    }
    // #3 Next node is small cave
    //      && there is already some small cave that has been visited twice
    //      && this particular small cave has been already visited at least once
    // Then throw away this path
    else if(
        nextNode.toLowerCase() === nextNode
        &&
        path.some((node, i, pathArr) => 
            pathArr.indexOf(node) < i && 
            node.toLowerCase() === node
        )
        && path.filter(node => node === nextNode).length > 0
    ) {
        return
    }
